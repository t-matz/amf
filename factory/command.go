...
func ensureLogger() {
	...
	AmfConfig.Logger.PathUtil = &logger_util.LogSetting{}
	AmfConfig.Logger.OpenApi = &logger_util.LogSetting{}
}
...
func loggerPathUtilDebugLevel(com int, args cmd.Args) int {
	level := args[0].(string)
	switch com {
	case cmd.Set:
		AmfConfig.Logger.PathUtil.DebugLevel = level
	case cmd.Delete:
		AmfConfig.Logger.PathUtil.DebugLevel = ""
	}
	return cmd.Success
}

func loggerPathUtilReportCaller(com int, args cmd.Args) int {
	str := args[0].(string)
	enable := booleanParse(str)
	switch com {
	case cmd.Set:
		AmfConfig.Logger.PathUtil.ReportCaller = enable
	case cmd.Delete:
		AmfConfig.Logger.PathUtil.ReportCaller = false
	}
	return cmd.Success
}

func loggerOpenapiDebugLevel(com int, args cmd.Args) int {
	level := args[0].(string)
	switch com {
	case cmd.Set:
		AmfConfig.Logger.OpenApi.DebugLevel = level
	case cmd.Delete:
		AmfConfig.Logger.OpenApi.DebugLevel = ""
	}
	return cmd.Success
}

func loggerOpenapiReportCaller(com int, args cmd.Args) int {
	str := args[0].(string)
	enable := booleanParse(str)
	switch com {
	case cmd.Set:
		AmfConfig.Logger.OpenApi.ReportCaller = enable
	case cmd.Delete:
		AmfConfig.Logger.OpenApi.ReportCaller = false
	}
	return cmd.Success
}
...
func dumpPlmn() {
	for _, plmn := range AmfConfig.Configuration.PlmnSupportList {
		for _, snssai := range plmn.SNssaiList {
			fmt.Println("XXX snssai", snssai.Sst, snssai.Sd)
		}
	}
}
...
func commandInit() {
	...
	parser.InstallCmd([]string{"amf", "logger", "path-util", "debug-level", "WORD"}, loggerPathUtilDebugLevel)
	parser.InstallCmd([]string{"amf", "logger", "path-util", "report-caller", "WORD"}, loggerPathUtilReportCaller)

	parser.InstallCmd([]string{"amf", "logger", "openapi", "debug-level", "WORD"}, loggerOpenapiDebugLevel)
	parser.InstallCmd([]string{"amf", "logger", "openapi", "report-caller", "WORD"}, loggerOpenapiReportCaller)
